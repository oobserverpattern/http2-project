...
public class BufferBasedPolicyWithHTTP1_1 implements SchedulePolicy {

	public BufferBasedPolicyWithHTTP1_1(ParseMPD mpd, Buffer buffer) {
		this.mpd = mpd;
		this.buffer = buffer;
	}
	...
	public String[] nextLayerToDownloader() {
		...
		// 如果buffer长度大于等于buffer最小值 小于等于buffer阈值//没有加smooth 改为小于buffer长度
		// 质量提升或下降
...
//				&& buffer.getAvail_seg() <= buffer.getMaxSize()) 
			...
			if (chunkLayer == 0) {
				...
				if (buffer.getAvail_seg() > Vup && last_down_bandwidth > bandwidthLevelPlus) {
					System.out.println("上一次下载的是l0>>>质量提升>>>下载带宽是:"+last_down_bandwidth+">>>质量提升所需带宽为:"+bandwidthLevelPlus);
					segID = segID + 1;
					chunkLayer = 1;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
				...
				// else 只能质量不变 因为下载是的l0
					...
					System.out.println("66666上一次下载的是l0>>>质量不变>>>下载带宽是:"+last_down_bandwidth+"质量提升所需带宽为:"+bandwidthLevelPlus);
					segID = segID + 1;
					chunkLayer = 0;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
			...
			if (chunkLayer == 1) {
				...
				// 两个条件都满足，质量提升，下载l2层
				if (buffer.getAvail_seg() > Vup && last_down_bandwidth > bandwidthLevelPlus) {
					System.out.println("上一次下载的是l1>>>质量提升>>>下载带宽是:"+last_down_bandwidth+">>>质量提升所需带宽为:"+bandwidthLevelPlus);
					segID = segID + 1;
					chunkLayer = 2;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
				...
				// 满足以下条件 质量下降
					...
					System.out.println("上一次下载的是l1>>>质量下降>>>下载带宽是:"+last_down_bandwidth+">>>质量下降满足的带宽要求为"+bandwidththisLevel);
					segID = segID + 1;
					chunkLayer = 0;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
				...
				// 否砸质量不变
					...
					System.out.println("上一次下载的是l1>>>质量不变>>>下载带宽是>>>"+last_down_bandwidth+">>>质量提升所需带宽为:"+bandwidthLevelPlus+"质量下降满足的带宽要求为"+bandwidththisLevel);
					segID = segID + 1;
					chunkLayer = 1;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
			...
			if (chunkLayer == 2) {
				...
				if (buffer.getAvail_seg() < Vdown || last_down_bandwidth < bandwidththisLevel) {
					System.out.println("上一次下载的是l2>>>质量下降>>>下载带宽是:"+last_down_bandwidth+"质量提升所需带宽为:"+bandwidththisLevel);
					segID = segID + 1;
					chunkLayer = 1;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
	...
	// 如果buffer长度大于buffer阈值
	// smooth
//		else if(buffer.getAvail_seg() > Integer.parseInt(MyPropertiesPro.getProperty("conf.threshold_buffer")))
//		{
//			
//			
//		}
