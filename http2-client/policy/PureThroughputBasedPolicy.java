...
public class PureThroughputBasedPolicy implements SchedulePolicy {
	public PureThroughputBasedPolicy(ParseMPD mpd) {
		this.mpd = mpd;
	}
	...
	public String[] nextLayerToDownloader() {
		...
		System.out
				.println("输出数据为" + ((Double.parseDouble(MyPropertiesPro.getProperty("conf.svc_total_size")) * 8 / 1024)
						/ (Double.parseDouble(MyPropertiesPro.getProperty("conf.segment_duration")))));
		System.out.println("上次下载带宽为" + last_down_bandwidth);
		// 如果当前带宽足够下载三层
