...
public class BufferBasedPolicyWithServerPushAndSizePredict implements SchedulePolicy {

	public BufferBasedPolicyWithServerPushAndSizePredict(ParseMPD mpd, Buffer buffer) {
		this.mpd = mpd;
		this.buffer = buffer;
		//this.sizepredict = new SizePredictor();
	}
	...
	public String[] nextLayerToDownloader() {
		...
		//开始先下载10个完整的segment作为大小预测的样本
		if(segID < 15)
		{
			segID = segID + 1;
			chunkLayer = 2;
			System.out.println("下载前10个完整块作为样本。当前下载块ID是"+segID);
...
//						* Double.parseDouble(MyPropertiesPro.getProperty("conf.threshold_buffer"));
				//预测出大小。分子：l0大小+l1大小  分母：l0大小+l1大小+l2大小（三层总大小）
				Vup = ((size + SizePredictor.getPredictedNum(SizePredictor.getList1(), size))/
						(size + SizePredictor.getPredictedNum(SizePredictor.getList1(),size) + SizePredictor.getPredictedNum(SizePredictor.getList1(),size)))
						* Double.parseDouble(MyPropertiesPro.getProperty("conf.threshold_buffer"));
...
//						/ (Double.parseDouble(MyPropertiesPro.getProperty("conf.segment_duration")) * 1024);
				...
				bandwidthLevelPlus = (size + SizePredictor.getPredictedNum(SizePredictor.getList1(), size)) * 8
						/ (Double.parseDouble(MyPropertiesPro.getProperty("conf.segment_duration")) * 1000);
				System.out.println("预测出的l1层大小是"+SizePredictor.getPredictedNum(SizePredictor.getList1(), size));
				System.out.println("预测出的l2层大小是"+SizePredictor.getPredictedNum(SizePredictor.getList2(), size));
				System.out.println("当前buffer长度是："+buffer.getAvail_seg()+"质量提升的buffer阈值是："+Vup);
				System.out.println("当前带宽是："+last_down_bandwidth+"质量提升的带宽阈值是："+bandwidthLevelPlus);
				
				// 两个条件都满足，质量提升，下载l1层
				if (buffer.getAvail_seg() > Vup && last_down_bandwidth > bandwidthLevelPlus) {
					System.out.println(
							"**********上一次下载的是l0>>>质量提升>>>下载带宽是:" + last_down_bandwidth + ">>>质量提升所需带宽为:" + bandwidthLevelPlus);
					segID = segID + 1;
					chunkLayer = 1;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
				...
				// else 只能质量不变 因为下载是的l0
					...
					System.out.println(
							"**********上一次下载的是l0>>>质量不变>>>下载带宽是:" + last_down_bandwidth + "质量提升所需带宽为:" + bandwidthLevelPlus);
			...
			if (chunkLayer == 1) {
				...
				Vdown = (size + SizePredictor.getPredictedNum(SizePredictor.getList1(), size))
						/ (size + SizePredictor.getPredictedNum(SizePredictor.getList1(),size) + SizePredictor.getPredictedNum(SizePredictor.getList2(),size))
						* Double.parseDouble(MyPropertiesPro.getProperty("conf.threshold_buffer"));
				
				// 层次加1 也就是三层的总下载带宽
				bandwidthLevelPlus = (size + SizePredictor.getPredictedNum(SizePredictor.getList1(),size) + SizePredictor.getPredictedNum(SizePredictor.getList2(),size))
						* 8 / (Double.parseDouble(MyPropertiesPro.getProperty("conf.segment_duration")) * 1000);

				bandwidththisLevel = (size + SizePredictor.getPredictedNum(SizePredictor.getList1(), size)) * 8
						/ (Double.parseDouble(MyPropertiesPro.getProperty("conf.segment_duration")) * 1000);
				System.out.println("预测出的l1层大小是"+SizePredictor.getPredictedNum(SizePredictor.getList1(), size));
				System.out.println("预测出的l2层大小是"+SizePredictor.getPredictedNum(SizePredictor.getList2(), size));
				System.out.println("当前buffer长度是："+buffer.getAvail_seg()+"质量提升的buffer阈值是："+Vup);
				System.out.println("当前buffer长度是："+buffer.getAvail_seg()+"质量下降的buffer阈值是："+Vdown);
				System.out.println("当前带宽是："+last_down_bandwidth+"质量提升的带宽阈值是："+bandwidthLevelPlus);
				System.out.println("当前带宽是："+last_down_bandwidth+"质量下降的带宽阈值是："+bandwidththisLevel);
				// 两个条件都满足，质量提升，下载l2层
				if (buffer.getAvail_seg() > Vup && last_down_bandwidth > bandwidthLevelPlus) {
					System.out.println(
							"**********上一次下载的是l1>>>质量提升>>>下载带宽是:" + last_down_bandwidth + ">>>质量提升所需带宽为:" + bandwidthLevelPlus);
					segID = segID + 1;
					chunkLayer = 2;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
				...
				// 满足以下条件 质量下降
					...
					System.out.println(
							"**********上一次下载的是l1>>>质量下降>>>下载带宽是:" + last_down_bandwidth + ">>>质量下降满足的带宽要求为" + bandwidththisLevel);
					segID = segID + 1;
					chunkLayer = 0;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
			...
			if (chunkLayer == 2) {
				...
				bandwidththisLevel = (size + SizePredictor.getPredictedNum(SizePredictor.getList1(),size) + SizePredictor.getPredictedNum(SizePredictor.getList2(),size))
						* 8 / (Double.parseDouble(MyPropertiesPro.getProperty("conf.segment_duration")) * 1000);
				...
				if (buffer.getAvail_seg() < Vdown || last_down_bandwidth < bandwidththisLevel) {
					System.out.println(
							"**********上一次下载的是l2>>>质量下降>>>下载带宽是:" + last_down_bandwidth + "质量提升所需带宽为:" + bandwidththisLevel);
					segID = segID + 1;
					chunkLayer = 1;
					result[0] = String.valueOf(segID);
					result[1] = String.valueOf(chunkLayer);
	...
	//private SizePredictor sizepredict;
	// 下载信息
	// 上一次的下载带宽
